...
func HTML(HandleFunc http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Security-Policy", "default-src 'none'; script-src 'self' 'unsafe-inline' unpkg.com; media-src http://sverigesradio.se *.sr.se; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self'; frame-src 'self'; frame-ancestors 'self'; block-all-mixed-content; upgrade-insecure-requests;")
		HandleFunc(w, r)
	}
}
